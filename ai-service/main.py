...
def extract_expiry(req: OcrRequest):
    try:
        ...
        # Strict Regex for DD/MM/YYYY, MM/YYYY, YYYY-MM-DD
        date_pattern = r'\b(?:(?:0[1-9]|[12][0-9]|3[01])[-/](?:0[1-9]|1[0-2])[-/]\d{4}|(?:0[1-9]|1[0-2])[-/]\d{4}|\d{4}[-/](?:0[1-9]|1[0-2])[-/](?:0[1-9]|[12][0-9]|3[01]))\b'
        match = re.search(date_pattern, text)
        
        if match:
            return {"expiryDate": match.group(0), "isValid": True}
        else:
            return {"expiryDate": None, "isValid": False, "message": "No strict date structure detected in image."}
    except Exception as e:
